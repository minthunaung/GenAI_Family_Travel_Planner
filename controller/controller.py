...
from agents.seed_data_agent import seed_data_agent
from agents.create_rag_agent import create_rag_agent
...
def build_travel_planner_graph_with_router():
    ...
    # 2) Task nodes
    graph.add_node("seed_data",       RunnableLambda(seed_data_agent(parent_dir+"/family_travel_planner.db")))
    graph.add_node("create_rag",      RunnableLambda(create_rag_agent(parent_dir+"/family_travel_rag.index")))    
    ...
    graph.add_node("photo_memory",    RunnableLambda(photo_memory_agent(parent_dir+"/family_travel_planner.db")))
    ...
    graph.add_conditional_edges(
      ...
      lambda s: s.next
                if s.next in {
                  "seed_data", #Update family members for trip
                  "create_rag", #upload trip pdf or html link to rag database
    ...
    #graph.add_edge("seed_data",   "summarize")
    #graph.add_edge("create_rag",   "summarize")
